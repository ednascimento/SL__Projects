...
$query = '';
$output = array();
$query .= "SELECT * FROM sl_projects WHERE is_intern = 0 ";
if(isset($_POST["search"]["value"]))
{
    /*TODO fix search engine */
	/*$query .= 'OR sl_nr LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR project LIKE "%'.$_POST["search"]["value"].'%" ';*/

    /* $query .= 'OR sl_budget LIKE "%'.$_POST["search"]["value"].'%" ';*/
...
foreach($result as $row)
{
    ...
    if($row["sl_nr"] != '')
    {
        $slNr = '<td >'. $row["sl_nr"]. '<div id="'.$row["id"].'" class="sl-extern ui-sortable-handle " ></div>' .'</td>';
    }
    else
    {
        $slNr = '';
